...
function setupHandlers(app) {
  ...
  app.get("/upload", (req, res) => {
    http.request({
      ...
      response.on("end", () => {
        res.render("upload-video", { product: JSON.parse(product) });
      })
      response.on("error", (err) => {
        console.error("Failed to get product list.");
        console.error(err || `Status code: ${response.statusCode}`);
  ...
  app.get("/history", (req, res) => {
    http.request({
      ...
      let product = ""
      response.on("data", chuck => { product += chuck })
      response.on("end", () => {
        http
          .request(
            ...
            (response) => {
              ...
              response.on("end", () => {
                ...
                res.render("history", { videos: JSON.parse(data).videos, product: JSON.parse(product) });
              });
